...
**디자인 QA 플러그인**. Figma 디자인과 로컬 웹 구현이 일치하는지 비교해 한 장짜리 리포트를 만든다.

## 시작하기

```bash
npm start   # Playwright 자동 설치(최초 1회) + 웹 런처 오픈 → http://localhost:4567
```

런처가 열리면 Claude Code 창에서 `/loop 1m /qa-watch`를 한 번 입력해 감시 루프를 켠다.
이후 런처에서 Figma URL·웹 URL을 입력하고 버튼을 누르면 QA가 자동으로 실행된다.

- **서버가 이미 실행 중이면** `npm start`는 중복 실행 없이 브라우저만 연다.
- `start.command`를 더블클릭해도 동일하게 동작한다.
