...
> ⚠️ **qa-watch에서 호출된 경우**: 모든 입력(`figmaUrl`, `webUrl`, `width`, `scale`)이 이미 전달됐으므로 이 1단계 전체를 건너뛰고 즉시 2단계로 간다. 사용자에게 아무것도 묻지 않는다.

- **먼저 프리셋 확인** — `.claude/skills/design-qa/presets.json` 을 Read한다. 사용자가 화면 **이름**(예: "결제창 QA 해줘", "신맞고스토어 비교")만 줬고 그 이름이 `presets`의 키와 (부분) 일치하면, 저장된 `figma`/`web`/`width`/`label`을 입력으로 쓴다.
  - 프리셋의 `web`이 비어 있거나, `note`에 토큰 만료/인증 주의가 있으면 → 사용자에게 유효한 웹 URL을 확인받고 진행(추측 금지).
  - 사용자가 URL을 직접 주면 그게 우선(프리셋보다). 이름+URL을 같이 주면 URL로 덮어쓴다.
  - **새 화면을 URL로 처음 QA**했고 사용자가 "프리셋에 저장해줘"라고 하면 presets.json에 항목을 추가한다(이름·figma·web·width·label).
- **Figma 프레임 URL**(node-id 포함), **로컬 웹 URL**. 프리셋도 직접 입력도 없으면 사용자에게 요청.
- 단일인지 **반응형(멀티 브레이크포인트)**인지 확인. **figmaUrl과 webUrl이 이미 주어졌으면 단일 모드로 간주하고 확인하지 않는다.**
...
프롬프트에 경로 전달: `figmaCodePath=reports/figma-code.txt`, `figmaMetaPath=reports/figma-meta.xml`, `figmaTokensPath=reports/figma-tokens.json`(있으면), `figmaImagePath=reports/figma.png`(없으면 "이미지 없음" 명시), `webUrl`, `width`, `scale`, `outPrefix=reports/web`, `reportPath=reports/qa-report.html`, `label`(선택).
