...
- `{{DIFF_SECTION_STYLE}}`: **반드시 `display:none`** — visual 모드는 차이 목록 테이블 전체를 숨긴다. 절대 빈 문자열로 두지 말 것.
- `{{DIFF_ROWS}}`: **반드시 빈 문자열** — 어떤 내용도 쓰지 말 것. ⚠️ DOM이 없는 상태에서 CSS 클래스명(`.foo`, `#bar`)을 추측해 써넣는 것은 허위 정보다. 절대 금지.
- `{{NEAR_MATCH_LIST}}`: **빈 문자열** — 작성 금지.
- `{{CHECK_LIST}}`: **빈 문자열** — 작성 금지.
- `{{DETAIL_SPINNER_SECTION}}`: **빈 문자열** — visual 모드는 스피너 없음.

> ⚠️ **로케이터 규칙 (visual / phase1 / webImageProvided 공통)**: DOM을 캡처하지 않은 상태에서 CSS 클래스명·ID(`.payment-guide`, `#btn-buy` 등)를 쓰는 것은 **허위 정보**다. 로케이터는 반드시 **Figma 레이어명**(figma-meta.xml의 `name` 속성, 예: `pay_general`, `gnb`)이나 **시각적 설명**("하단 구매 버튼", "한코인 결제 박스")으로만 쓴다.

### 4P1. 리포트 작성 (phase1 모드) — 스피너 포함, 빠르게

4V와 동일하게 작성하되, `{{DETAIL_SPINNER_SECTION}}` 만 다르게 채운다.

**▶ `webImageProvided: true`인 경우 (스크린샷 모드) — 스피너 없음:**

```html
<div style="margin:2rem auto;max-width:800px;padding:1.25rem 1.5rem;background:var(--surface,#f8f8f8);border-radius:8px;color:var(--muted);font-size:.875rem;text-align:center">
  📷 스크린샷 모드 — 실제 웹 URL이 없어 CSS 속성 분석을 수행할 수 없습니다.<br>웹 URL을 직접 입력하면 CSS 상세 비교가 가능합니다.
</div>
```

- `{{CHIPS}}`: `<span class="chip">⚡ 시각 비교</span><span class="chip gray">📷 스크린샷 모드</span>`
- `{{SUMMARY}}` 끝에: `<div class="caveats"><span class="badge">📷 스크린샷 모드 — CSS 속성 분석 불가 (웹 URL 필요)</span></div>`

**▶ `webImageProvided: false`인 경우 (웹 URL 캡처 모드) — 스피너:**
